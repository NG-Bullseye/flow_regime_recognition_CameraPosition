    ...
    def parse_data_points(self):
        for file in os.listdir(self.path_to_data):
            if os.path.isfile(os.path.join(self.path_to_data, file)) and file.endswith('.png'):
                ...
                pathToJson = os.path.join(self.path_to_data, jsonFileName_metadata_WITHOUT_CAMERA_FRAME+'.json')

                if os.path.isfile(pathToJson):
                    data_point = DataPoint(filename, filename_image, pathToJson)
...
class ImageDataExtractor:
    ...
    def preprocess_image(self, img):
        ...
        img = img / 255
        #dataaugmentation implementation. Spiegeln, drehen.
        #filter sharpening
        return img
...
class LabelExtractor:
    ...
    def extract_label(self, metadata):
        label_int = metadata[self.label_name]['parameters']['value']
